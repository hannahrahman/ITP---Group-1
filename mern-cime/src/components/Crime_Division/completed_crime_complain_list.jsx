...
export default class CompletedCrimeComplainList extends Component {

    constructor(props) {
        super(props)
        this.state = {
            completedCrimeComplains: [],
            currentPage: 1,
            recordsPerPage: 10,
            searchResults: [],
        };
    ...
    findAllComplains() {
        axios.get('http://localhost:5000/Completecomplain/')
            .then(res => res.data)
            .then((data) => {
                this.setState({
                    completedCrimeComplains: data
                });
            });
    };
    ...
    lastPage = () => {
        if (this.state.currentPage < Math.ceil(this.state.completedCrimeComplains.length / this.state.recordsPerPage)) {
            this.setState({
                currentPage: Math.ceil(this.state.completedCrimeComplains.length / this.state.recordsPerPage)
            });
        }
    };
    nextPage = () => {
    ...
    searchRecords(e) {
        ...
        if (item.length >= 0) {
            ...
            if (this.state.searchResults) {
                ...
                this.setState({ completedCrimeComplains: itemArray })
            }
        }
    }
    render() {
        const { completedCrimeComplains, currentPage, recordsPerPage } = this.state;
        const lastIndex = currentPage * recordsPerPage;
        const firstIndex = lastIndex - recordsPerPage;
        const currentRecords = completedCrimeComplains.slice(firstIndex, lastIndex);
        const totalPages = completedCrimeComplains.length / recordsPerPage;
        const totalRecords = completedCrimeComplains.length;
            ...
            <div className="complain">
                <div class="card text-light  bg-dark  mb-3" style={{ marginLeft: 8.5 + 'rem' }} >
                    <div class="card-header">
                        <h3><FontAwesomeIcon icon={faListAlt} /> Completed Complains List</h3>
                    ...
                    <div class="card-body" >
                        <div className="table-wrapper ">
                            <Table striped border hover variant="dark">
                                ...
                                <tbody >
                                    {completedCrimeComplains.length === 0 ? <tr align="center">
                                        ...
                                        currentRecords.map((complain, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <Link className="btn btn-primary" to={`/ViewCompletedComplaint/${complain._id}`}>View</Link>
                                                </td>
                                            </tr>
                                        ))
                                    }
