...
export default class MiscellaneousComplainList extends Component {
    ...
    findAllComplains() {
        axios.get('http://localhost:5000/miscComplains/')
            .then(res => res.data)
            .then((data) => {
                this.setState({
                    miscComplains: data
    ...
    lastPage = () => {
        if (this.state.currentPage < Math.ceil(this.state.miscComplains.length / this.state.recordsPerPage)) {
            this.setState({
                currentPage: Math.ceil(this.state.miscComplains.length / this.state.recordsPerPage)
            });
        }
    };
    nextPage = () => {
    ...
    searchRecords(e) {
        ...
        if (item.length >= 0) {
            ...
            if (this.state.searchResults) {
                ...
                this.setState({ miscComplains: itemArray })
            }
        }
    }
    deleteComplain = (complainId) => {
        ...
        if (window.confirm("Do you want to delete this record?")) {
            axios.get('http://localhost:5000/miscComplains/update/' + complainId)
                .then(function (res) {
                    backupRecords = res.data;
                    axios.post('http://localhost:5000/Addcomplain_backup/add', backupRecords)
                });
            axios.get('http://localhost:5000/miscComplains/delete/' + complainId)
                .then(response => {
    ...
    render() {
        const { miscComplains, currentPage, recordsPerPage } = this.state;
        const lastIndex = currentPage * recordsPerPage;
        const firstIndex = lastIndex - recordsPerPage;
        const currentRecords = miscComplains.slice(firstIndex, lastIndex);
        const totalPages = miscComplains.length / recordsPerPage;
        const totalRecords = miscComplains.length;
            ...
            <div className="complain">
                <div class="card text-light  bg-dark  mb-3" style={{ marginLeft: 8.5 + 'rem' }} >
                    <div class="card-header">
                        <h3><FontAwesomeIcon icon={faListAlt} /> Complaints List</h3>
                    ...
                    <div class="card-body" >
                        <div className="table-wrapper ">
                            <Table striped border hover variant="dark">
                                ...
                                <tbody >
                                    {miscComplains.length === 0 ? <tr align="center">
                                        ...
                                        currentRecords.map((complain, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <Link className="btn btn-primary" to={`/EditMiscellaneousComplain/${complain._id}`}>View</Link>
                                                </td>
                                                <td>
