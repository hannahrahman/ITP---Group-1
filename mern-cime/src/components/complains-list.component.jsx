...
export default class CrimeComplainList extends Component {
    ...
    lastPage = () => {
        if (this.state.currentPage < Math.ceil(this.state.crimeComplains.length / this.state.recordsPerPage)) {
            this.setState({
                currentPage: Math.ceil(this.state.crimeComplains.length / this.state.recordsPerPage)
            });
        }
    };
    nextPage = () => {
    ...
    searchRecords(e) {
        ...
        if (item.length >= 0) {
            ...
            if (this.state.searchResults) {
                ...
                this.setState({ crimeComplains: itemArray })
            }
        }
    }
    ...
    render() {
        const { crimeComplains, currentPage, recordsPerPage } = this.state;
        const lastIndex = currentPage * recordsPerPage;
        const firstIndex = lastIndex - recordsPerPage;
        const currentRecords = crimeComplains.slice(firstIndex, lastIndex);
        const totalPages = crimeComplains.length / recordsPerPage;
        const totalRecords = crimeComplains.length;
                    ...
                    <div class="card-body" >
                        <div className="table-wrapper ">
                            <Table striped border hover variant="dark">
                                ...
                                <tbody >
                                    {crimeComplains.length === 0 ? <tr align="center">
                                        ...
                                        currentRecords.map((complain, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <Link className="btn btn-primary" to={`/EditCrimeComplain/${complain._id}`}>View</Link>
                                                </td>
                                                <td>
