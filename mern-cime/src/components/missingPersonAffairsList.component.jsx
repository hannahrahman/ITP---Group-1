...
export default class MissingPersonList extends Component {
  ...
  findAllComplains() {
    axios.get('http://localhost:5000/missingPersonAffairs/')
        ...
        .then((data) => {
            this.setState({missingPersonAffairs: data});
        });
};
...
lastPage = () => {
  if(this.state.currentPage < Math.ceil(this.state.missingPersonAffairs.length / this.state.recordsPerPage)) {
      this.setState({
          currentPage: Math.ceil(this.state.missingPersonAffairs.length / this.state.recordsPerPage)
      });
  }
};
...
searchRecords(e) {
  ...
  if(item.length >= 0){
      ...
      if(this.state.searchResults){
          ...
          this.setState({missingPersonAffairs:itemArray})
      }
  }
}
...
deleteComplain = (complainId) => {
  ...
  if(window.confirm("Do you want to delete this record?")) {
      axios.get('http://localhost:5000/missingPersonAffairs/edit/'+complainId)
      .then(function(res) {
          backupRecords = res.data;
          axios.post('http://localhost:5000/domestic_abuse_complains_backup/add', backupRecords)
      });
      axios.get('http://localhost:5000/missingPersonAffairshttp://localhost:5000/missingPersonAffairs/delete/'+complainId)
      .then(response => {
  ...
  render() {
    ...
    const {missingPersonAffairs, currentPage, recordsPerPage} = this.state;
        const lastIndex = currentPage * recordsPerPage;
        const firstIndex = lastIndex - recordsPerPage;
        const currentRecords = missingPersonAffairs.slice(firstIndex, lastIndex);
        const totalPages = missingPersonAffairs.length / recordsPerPage;
        const totalRecords = missingPersonAffairs.length;
                    ...
                    <div class="card-body" >
                        <div className="table-wrapper">
                            <Table striped border hover variant="dark">
                                ...
                                <tbody>
                                    {missingPersonAffairs.length === 0 ?
                                        <tr align="center">
                                            <td colSpan="6">No Complains Available</td>
                                        </tr> :
                                        currentRecords.map((missingPersonComplain, index) => (
                                            <tr key={index}>
                                                <td>{missingPersonComplain.mp_fullname}</td>
                                                <td>{missingPersonComplain.mp_nic}</td>
                                                <td>{missingPersonComplain.mp_address}</td>
                                                <td>
                                                    <Link className="btn btn-primary" to={`//editComplaint/${missingPersonComplain._id}`}>View</Link>
                                                </td>
                                                <td>
                                                    <Link className="btn btn-danger" onClick={this.deleteComplain.bind(this, missingPersonComplain._id)}>Delete</Link>
                                                </td>
                                            </tr>
                                        ))
                                    }
